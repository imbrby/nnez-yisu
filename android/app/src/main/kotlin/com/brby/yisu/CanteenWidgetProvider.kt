...
private object CanteenWidgetRenderer {
    fun update(
        ...
        val estimatedDays = if (prefs.contains("widget_estimated_days")) {
            prefs.getInt("widget_estimated_days", -1)
        } else {
            -1
        }
        ...
        views.setTextViewText(
            R.id.tv_balance,
            when (kind) {
                WidgetKind.ENDURANCE -> if (hideBalance) "余额 ••••" else "余额 ¥ $balance"
                else -> if (hideBalance) "••••" else "¥ $balance"
            }
        )
        ...
        when (kind) {
            WidgetKind.BALANCE -> {
                ...
                views.setTextColor(R.id.tv_balance_label, palette.muted)
            }
            WidgetKind.OVERVIEW -> {
                views.setTextColor(R.id.tv_accent, palette.accent)
                views.setTextViewText(
